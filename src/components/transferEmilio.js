...
const TransEmilio = () => {
      ...
      const transacciones = historialDoc.data().transacciones
        ? historialDoc
            ...
            .transacciones.filter(
              transaccion =>
                transaccion.jugador === "Emilio" && transaccion.cantidad !== 0
            )
        : []
  ...
  const handleActualizarSaldo = (jugadorId, nombreJugador) => {
    ...
    actualizarSaldoJugador("Jugador1", -cantidad).then(() => {
      ...
      actualizarSaldoJugador(jugadorId, cantidad).then(() => {
        ...
        agregarTransaccion("Emilio", nombreJugador, cantidad).then(() => {
  ...
  const handleBanco = () => {
    ...
    actualizarSaldoJugador("Jugador1", -cantidad).then(() => {
      ...
      agregarTransaccion("Emilio", "Banco", -cantidad).then(() => {
