...
const obtenerPropiedades = async () => {
    try {
      const jugadorRef = doc(app, "jugadores", "Jugador1")
  ...
  const transferirPropiedad = async (
    ...
    try {
      const emilioRef = doc(app, "jugadores", "Jugador1")
...
function TraspasarPropsEmilio() {
  ...
  <Dropdown.Menu>
    {jugadores.map((jugador, index) => (
      <Dropdown.Menu>
      {jugadores.filter(jugador => jugador.id !== "Jugador1").map((jugador, index) => (
        <Dropdown.Item
          key={index}
...
export default TraspasarPropsEmilio
