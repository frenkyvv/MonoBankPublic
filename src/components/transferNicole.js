...
const TransfersNicole = () => {
      ...
      const transacciones = historialDoc.data().transacciones
        ? historialDoc
            ...
            .transacciones.filter(
              transaccion =>
                transaccion.jugador === "Nicole" && transaccion.cantidad !== 0
            )
        : []
  ...
  const handleActualizarSaldo = (jugadorId, nombreJugador) => {
    ...
    actualizarSaldoJugador("Jugador2", -cantidad).then(() => {
      ...
      actualizarSaldoJugador(jugadorId, cantidad).then(() => {
        console.log("handleActualizarSaldo: saldo del jugador seleccionado actualizado")
        agregarTransaccion("Nicole", nombreJugador, cantidad).then(() => {
  ...
  const handleBanco = () => {
    ...
    actualizarSaldoJugador("Jugador2", -cantidad).then(() => {
      ...
      agregarTransaccion("Nicole", "Banco", -cantidad).then(() => {
  ...
  {historial &&
    historial
      ...
      .map((transaccion, index) => (
        <li key={index}>
          {new Date(transaccion.timestamp).toLocaleTimeString()} -{" "}
...
export default TransfersNicole
